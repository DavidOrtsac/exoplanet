#!/usr/bin/env python3
"""
COMPREHENSIVE 50-EXAMPLE TEST FOR EXOPLANET CLASSIFIER
=====================================================

This script performs a rigorous test of our Random Forest model on 50 held-out examples
that were never seen during training. It provides detailed analysis of:
1. Data verification (50 examples confirmed)
...
def load_test_data():
    ...
    # Use the 50-random sample we created
    test_file = '../data/test_50_random.csv'
    if not os.path.exists(test_file):
        print(f"❌ {test_file} not found")
        print("📁 Creating 50 random samples from heldout_test_rows.csv...")
        try:
            full_heldout = pd.read_csv('heldout_test_rows.csv')
            # Sample 50 random rows
            test_df = full_heldout.sample(n=50, random_state=42)
            # Save for future use
            test_df.to_csv(test_file, index=False)
            print(f"✅ Created {test_file} with 50 random samples")
...
def verify_test_data(df):
    """Verify we have exactly 50 examples and analyze the answer keys"""
    ...
    if n_examples < 50:
        print(f"⚠️  Warning: Only {n_examples} examples available (expected 50)")
    elif n_examples > 50:
        print(f"📝 Using first 50 examples from {n_examples} available")
        df = df.head(50)
    else:
        print("✅ Exactly 50 examples confirmed")
...
def run_predictions(model, scaler, df, label_col):
    ...
    # Handle missing values (use median imputation like in training)
    print("🔧 Handling missing values...")
    for col in features:
        if X_test[col].isna().any():
            median_val = X_test[col].median()
            X_test[col].fillna(median_val, inplace=True)
            print(f"   Filled {X_test[col].isna().sum()} missing values in {col}")
...
def analyze_results(results_df, pred_labels, actual_labels, confidences):
    ...
    if np.sum(high_conf_mask) > 0:
        ...
        print(f"High confidence (≥80%) predictions: {np.sum(high_conf_mask)}")
...
def save_detailed_results(results_df):
    ...
    output_file = 'test_50_detailed_results.csv'
    results_df.to_csv(output_file, index=False)
    print(f"\n💾 Detailed results saved to: {output_file}")

def main():
    """Main test execution"""
    print("🚀 COMPREHENSIVE 50-EXAMPLE EXOPLANET CLASSIFIER TEST")
    print("=" * 60)
    print("This test runs our Random Forest model on 50 held-out examples")
    ...
    print("=" * 60)
    print(f"✅ Test completed on 50 held-out examples")
    print(f"📊 Ground truth: {n_candidates} candidates, {n_false_positives} false positives")
    print(f"🎯 Overall accuracy: {metrics['overall_accuracy']:.1%}")
    print(f"🟢 Candidates correctly identified: {metrics['candidates_correct']}")
    print(f"🔴 Candidates incorrectly identified: {metrics['candidates_incorrect']}")
    print(f"🟢 False positives correctly identified: {metrics['fp_correct']}")
    print(f"🔴 False positives incorrectly identified: {metrics['fp_incorrect']}")
