...
export default function ThreeJSEarthBottomRight() {
  ...
  useEffect(() => {
    ...
    // Create camera positioned for bottom right view
    const camera = new THREE.PerspectiveCamera(75, w / h, 0.1, 1000);
    camera.position.x = 1.5; // Position camera for bottom right view
    camera.position.y = -1.5; // Lower position for bottom right
    camera.position.z = 2.5; // Closer to Earth for the smaller scale
    ...
    // Create Earth group positioned at bottom right
    const earthGroup = new THREE.Group();
    earthGroup.rotation.z = (-23.4 * Math.PI) / 180;
    earthGroup.position.x = 1.5; // Position Earth at bottom right
    earthGroup.position.y = -1.5; // Lower position
    earthGroup.position.z = 0;
    earthGroup.scale.setScalar(0.7); // Scale down to 70% as in the animation
